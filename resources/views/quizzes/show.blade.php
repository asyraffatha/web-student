...
<body class="bg-gray-100 py-10">
    ...
    @if ($type === 'daily')
        ...
        <div class="max-w-3xl mx-auto bg-white p-6 rounded-xl shadow-lg animate__animated animate__fadeInUp">
            ...
            <p class="mb-6 text-md text-gray-600">Passing Score: <span
                    class="font-semibold">{{ $quiz->passing_score }}</span></p>
            @if (session('error'))
            ...
            <form action="{{ route('quizzes.submit', $quiz->id) }}" method="POST">
                ...
                @foreach ($quiz->questions as $index => $question)
                    <div
                        class="mb-8 p-6 border border-gray-200 rounded-lg bg-gray-50/80 shadow-md animate__animated animate__fadeInUp animate__delay-{{ $index + 1 }}s">
                        ...
                        @if ($question->image)
                            <div class="mb-4 flex justify-center">
                                <img src="{{ asset('storage/' . $question->image) }}" alt="Gambar Pertanyaan"
                                    class="max-w-md rounded-lg shadow-md border-2 border-gray-300">
                            </div>
                        @endif
                        @if ($question->video)
                            <div class="mb-4 flex justify-center">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
                            ...
                            @foreach ($question->options as $i => $option)
                                <label
                                    class="relative flex items-center p-4 rounded-xl shadow-md cursor-pointer transition-transform hover:scale-105 {{ $optionColors[$i] }} text-white min-h-[60px]">
                                    <input type="radio" name="answers[{{ $question->id }}]"
                                        value="{{ ['A', 'B', 'C', 'D'][$i] }}"
                                        class="form-radio h-5 w-5 text-white bg-white border-white focus:ring-2 focus:ring-white mr-3"
                                        required>
                                    <span class="font-semibold text-lg">{{ $optionIcons[$i] }}
                                        {{ $option }}</span>
                                </label>
                            @endforeach
                        </div>
                ...
                <div class="text-right mt-8">
                    <button type="submit"
                        class="px-8 py-3 bg-blue-600 text-white rounded-full hover:bg-blue-700 transition duration-300 font-bold shadow-xl animate__animated animate__pulse animate__infinite">
                        Kirim Jawaban
                    </button>
                </div>
            </form>
    ...
    @elseif($type === 'teka-teki' || $type === 'boss')
        <style>
            body {
                background: linear-gradient(135deg, #a7d9f7 0%, #ffecb3 100%);
                /* Biru langit ke Kuning cerah */
                background-repeat: no-repeat;
                min-height: 100vh;
                font-family: 'Poppins', sans-serif;
                overflow-x: hidden;
                position: relative;
                /* Untuk background elements */
            }
            ...
            .quiz-container-gamify {
                ...
                /* Border ungu untuk kesan premium */
            ...
            .boss-quiz-theme {
                background: linear-gradient(135deg, #1a202c 0%, #4a0e4a 100%);
            ...
            @keyframes glowBorder {
                from {
                    box-shadow: 0 0 10px #ffeb3b, 0 0 20px #ffeb3b, 0 0 30px #ffeb3b, inset 0 0 10px #ffeb3b;
                }

                to {
                    box-shadow: 0 0 20px #ffeb3b, 0 0 30px #ffeb3b, 0 0 40px #ffeb3b, inset 0 0 20px #ffeb3b;
                }
            }
        ...
        @if ($type === 'teka-teki')
            ...
            @if ($hasFailedPreviously && !$isRetryRequested)
                <div
                    class="max-w-2xl mx-auto bg-gradient-to-r from-red-100 to-pink-100 border-l-4 border-red-500 text-red-700 p-8 rounded-2xl shadow-2xl animate__animated animate__fadeInDown">
                    <div class="text-center">
                        <div class="text-6xl mb-4 animate__animated animate__bounceIn">😢</div>
                        <p
                            class="font-bold text-3xl mb-4 text-center bg-clip-text text-transparent bg-gradient-to-r from-red-600 to-pink-600">
                            Ups! Belum Berhasil
                        </p>
            ...
            @elseif($isRetryRequested || !$hasFailedPreviously)
                <div
                    class="max-w-4xl mx-auto quiz-container-gamify animate__animated animate__zoomIn animate__delay-0.5s">
                    <!-- Progress Bar -->
                    <div class="progress-container mb-8">
                        <div class="progress-bar">
                            <div class="progress-fill"></div>
                        </div>
                        <div class="progress-text">Pertanyaan <span id="current-question">1</span> dari
                            {{ count($quiz->questions) }}</div>
                    </div>
                    ...
                    <div class="text-center mb-8">
                        ...
                        @if ($isRetryRequested)
                            <div
                                class="bg-gradient-to-r from-yellow-100 to-orange-100 border-l-4 border-yellow-500 text-yellow-800 p-6 rounded-xl mb-6 animate__animated animate__fadeIn">
                                <div class="flex items-center justify-center gap-3 mb-2">
                                    <span class="text-4xl">⚠️</span>
                                    <p class="font-bold text-2xl">Kesempatan Terakhir!</p>
                                </div>
                                <p class="text-xl">Ini adalah kesempatan terakhirmu untuk menyelesaikan teka-teki ini!
                                    Semangat! 💪</p>
                    ...
                    @if (session('error'))
                        <div
                            class="mb-6 p-6 bg-red-100 text-red-800 rounded-xl animate__animated animate__shakeX border-l-4 border-red-500">
                    ...
                    <form action="{{ route('quizzes.submit', $quiz->id) }}" method="POST" id="quizForm">
                        ...
                        @foreach ($quiz->questions as $index => $question)
                            <div class="question-container mb-12 p-8 border-4 border-pink-300 rounded-3xl bg-white/95 shadow-2xl animate__animated animate__fadeInUp animate__delay-{{ $index + 1 }}s hover:shadow-pink-200 transition-all duration-300"
                                data-question="{{ $index + 1 }}">
                                <div class="flex items-center gap-4 mb-6">
                                    <span
                                        class="text-4xl text-purple-600 animate__animated animate__swing animate__infinite animate__slow">🔎</span>
                                    <p class="font-extrabold text-pink-800 text-2xl">{{ $index + 1 }}.
                                        {{ $question->question }}</p>
                                </div>

                                @if ($question->image)
                                    <div
                                        class="mb-8 flex justify-center transform hover:scale-105 transition-transform duration-300">
                                        <img src="{{ asset('storage/' . $question->image) }}" alt="Gambar Pertanyaan"
                                            class="question-img-gamify rounded-2xl">
                                    </div>
                                @endif
                                ...
                                @if ($question->video)
                                    <div class="mb-8 flex justify-center">
                                        <video controls
                                            class="max-w-md rounded-2xl shadow-xl border-4 border-pink-300 hover:shadow-pink-200 transition-all duration-300">
                                            <source src="{{ asset('storage/' . $question->video) }}"
                                                type="video/mp4">
                                            Browser Anda tidak mendukung video.
                                        </video>
                                    </div>
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-8">
                                    ...
                                    @foreach ($question->options as $i => $option)
                                        <label
                                            class="option-gamify {{ $optionColors[$i] }} animate__animated animate__fadeInRight animate__delay-{{ $index * 0.2 + $i * 0.1 }}s transform hover:scale-105 transition-all duration-300 cursor-pointer">
                                            <input type="radio" name="answers[{{ $question->id }}]"
                                                value="{{ ['A', 'B', 'C', 'D'][$i] }}"
                                                class="form-radio h-6 w-6 text-white bg-white border-white focus:ring-3 focus:ring-yellow-300 mr-4"
                                                required onchange="playSelectSound()">
        ...
        @else<!-- $type === 'boss' -->
            ...
            <body class="boss-quiz-theme">
                <div class="max-w-4xl mx-auto boss-quiz-container animate__animated animate__fadeInDownBig">
                    <h1
                        class="text-5xl font-extrabold text-yellow-400 text-center mb-8 drop-shadow-xl animate__animated animate__flash animate__infinite animate__slow">
                        <span class="animate__animated animate__wobble animate__infinite">👑</span> BOSS QUIZ: <span
                            class="text-purple-400">{{ $quiz->title }}</span>
                    </h1>
                    ...
                    <form action="{{ route('quizzes.submit', $quiz->id) }}" method="POST">
                        ...
                        @foreach ($quiz->questions as $index => $question)
                            <div
                                class="mb-12 p-10 border-4 border-yellow-500 rounded-3xl bg-gray-900/80 shadow-2xl animate__animated animate__fadeInUp animate__delay-{{ $index + 1 }}s">
                                <p class="font-extrabold text-yellow-400 mb-8 text-2xl flex items-center gap-4">
                                    <span
                                        class="text-4xl text-red-500 animate__animated animate__jello animate__infinite">⚔️</span>
                                    {{ $index + 1 }}. {{ $question->question }}
                                </p>
                                @if ($question->image)
                                    <div class="mb-8 flex justify-center">
                                        <img src="{{ asset('storage/' . $question->image) }}" alt="Gambar Pertanyaan"
                                            class="question-img-gamify border-red-600">
                                    </div>
                                @endif
                                @if ($question->video)
                                    <div class="mb-8 flex justify-center">
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-8 mt-8">
                                    ...
                                    @foreach ($question->options as $i => $option)
                                        <label
                                            class="option-gamify boss-option-colors {{ $i % 2 === 0 ? 'animate__animated animate__fadeInLeft' : 'animate__animated animate__fadeInRight' }} animate__delay-{{ $index * 0.2 + $i * 0.1 }}s"
                                            style="{{ $bossOptionColors[$i] }}">
                                            <input type="radio" name="answers[{{ $question->id }}]"
                                                value="{{ ['A', 'B', 'C', 'D'][$i] }}"
                                                class="form-radio h-7 w-7 text-white bg-white border-white focus:ring-4 focus:ring-yellow-400 mr-4"
                                                required>
                        ...
                        <div class="text-center mt-12">
                            <button type="submit"
                                class="px-12 py-5 bg-yellow-500 text-gray-900 rounded-full hover:bg-yellow-400 transition duration-300 font-extrabold text-2xl shadow-2xl animate__animated animate__heartBeat animate__infinite animate__slower">
