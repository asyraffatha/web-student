...
<head>
    ...
    <title>Daftar Akun - Mathporia</title>
...
<body class="min-h-screen flex items-center justify-center px-4 overflow-hidden relative">
    ...
    <!-- Register Form Card -->
    <div class="register-container w-full max-w-md rounded-2xl p-8 space-y-6">
        <div class="text-center">
            ...
            <p class="mt-3 text-gray-600">Silakan isi data diri Anda dengan benar</p>
        </div>

        @if (session('success'))
            <div class="bg-green-100 text-green-700 text-sm p-3 rounded-lg text-center mb-4">
                {{ session('success') }}
            </div>
        @endif
        ...
        <form method="POST" action="{{ route('register') }}" class="space-y-6">
            ...
            <div class="input-field">
                ...
                <input type="text" id="name" name="name" value="{{ old('name') }}" required
                    class="block w-full px-4 py-3 rounded-lg focus:outline-none" placeholder="Masukkan nama lengkap">
                @error('name')
            ...
            <div class="input-field">
                ...
                <select id="kelas" name="kelas" required
                    class="block w-full px-4 py-3 rounded-lg border-2 border-gray-200 focus:outline-none focus:border-indigo-500 bg-white text-gray-700">
                    <option value="" disabled {{ old('kelas') ? '' : 'selected' }}>Pilih Kelas</option>

                    @foreach ([7, 8, 9] as $tingkat)
                        @foreach (range(1, 9) as $sub)
                            @php $value = $tingkat . '.' . $sub; @endphp
            ...
            <div>
                <button type="submit"
                    class="register-btn w-full bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white font-bold py-3 px-4 rounded-lg transition duration-300 transform">
                    Daftar Sekarang
                </button>
            </div>
        </form>
